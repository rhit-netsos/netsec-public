...
to reach a non-existing server. For this exeperiment, we will need two
...
We will repeat the setup for exeperiment 1, but this time, we will close the
...
2. What does the server do when it receive a conection termination request
   from the client?
...
(it will be simply echo'ed on the standard output at the server's end). To keep
...
3. For the packets between the connection establishment and teardown, fill out
...
On the client side, start a ternet session as follows:
```sh
telnet server
```
